...
import com.example.cryptonow.databinding.TopCurrencyLayoutBinding
...
class TopMarketAdapter(var contex : Context, val list: List<CryptoCurrency>) : RecyclerView.Adapter<TopMarketAdapter.TopMarketViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TopMarketViewHolder {
        return TopMarketViewHolder(LayoutInflater.from(contex).inflate(R.layout.top_currency_layout, parent,false))
    }
    ...
    override fun onBindViewHolder(holder: TopMarketViewHolder, position: Int) {
       val item = list[position]
        ...
        if (item.quotes!![0].percentChange24h > 0){
            ...
            holder.binding.topCurrencyChangeTextView.setTextColor(contex.resources.getColor(R.color.green))
            holder.binding.topCurrencyChangeTextView.text= "+ ${String.format("%.02f",item.quotes[0].percentChange24h)} %"

        }
        else{
            holder.binding.topCurrencyChangeTextView.setTextColor(contex.resources.getColor(R.color.red))
            holder.binding.topCurrencyChangeTextView.text= " ${String.format("%.02f",item.quotes[0].percentChange24h)} %"
        }
